...
*/
  ...
  private def printDelay(m: Module, filename: String = "delay.rpt"): Unit = {
    ...
    m bfs { node =>
      report.append(getNodeName(node) + ":" + nodeToString(node) + " ==> delay: " + (node.indelay + node.outdelay).toString + "\n")
    }
   
    report.append("\nCritical path delay: " + calcCPDelay(m))
