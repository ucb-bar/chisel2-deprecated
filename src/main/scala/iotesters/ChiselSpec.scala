...
trait ChiselRunners extends Assertions {
  val backends = TesterDriver.backends
  implicit val testArgs = Driver.testArgs
  def runTester(t: => BasicTester): Boolean = {
    TesterDriver.execute(() => t)
  }
  def assertTesterPasses(t: => BasicTester): Unit = {
    assert(runTester(t))
  }
  def assertTesterFails(t: => BasicTester): Unit = {
    assert(!runTester(t))
  }
