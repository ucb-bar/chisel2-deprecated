...
object SCWrapper {
   ...
   def main(args: Array[String]){
      //Read in template
      val fileContents = read_file("src/main/sysc/template.txt")

      //Generate replacements
      val def = example_component_def2()
      val replacements = generate_replacements(def)
   ...
   def example_component_def(): ComponentDef {
      ...
      cdef.ctype = "GCD_t";
      cdef.name = "GCD";
      cdef.entries.add(new CEntry("a", true, "dat_t<1>", "GCD__io_a", "GCD__io_r1", "GCD__io_v1"));
      cdef.entries.add(new CEntry("z", false, "dat_t<1>", "GCD__io_z", "GCD__io_rz", "GCD__io_vz"));
      return cdef;
   }

   def example_component_def2(): ComponentDef {
      val cdef = new ComponentDef();
      cdef.ctype = "AddFilter_t";
      cdef.name = "AddFilter";
      cdef.entries.add(new CEntry("a", true, "dat_t<16>", "AddFilter__io_a", "AddFilter__io_ar", "AddFilter__io_av"));
      cdef.entries.add(new CEntry("b", false, "dat_t<16>", "AddFilter__io_b", "AddFilter__io_br", "AddFilter__io_bv"));
      return cdef;
   }
    */
    ...
    def generate_replacements(c: ComponentDef):  Array[Array[String]]{
