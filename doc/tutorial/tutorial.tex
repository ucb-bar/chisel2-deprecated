...
A user first defines the complete interface to say a ROM as follows:
...
class RamIo extends RomIo {
  ...
  val wdata = Bits(width = 32, dir = INPUT)
}
\end{scala}
...
\noindent
and the control path component can be built by partially assigning to
...
\noindent
We can now wire up the CPU using bulk connects of as we would with other bundles:
...
\noindent
where repeated bulk connections of partially assigned interfaces
completely connects up destination interface.
...
\begin{scala}
  class decoupledDemo 
    extends ioDecoupled()( new DataBundle )
\end{scala}
