...
\begin{scala}
def risingedge(x: Bool) = x && !RegUpdate(x)
\end{scala}
...
val pcReg    = RegUpdate(pcNext, resetVal = UInt(0, 32))
...
def delays[T <: Data](x: T, n: Int): List[T] = 
  if (n <= 1) List(x) else x :: taps(RegUpdate(x), n-1)
...
class Fifo[T <: Data] (n: Int, type: T) 
  ...
  val enq_ptr      = RegReset(UInt(0, sizeof(n)))
  val deq_ptr      = RegReset(UInt(0, sizeof(n)))
  val is_full      = RegReset(Bool(false))
