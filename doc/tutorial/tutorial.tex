...
\begin{figure}
\begin{center}
\includegraphics[width=0.45\textwidth]{figs/CUT.pdf}
\end{center}
...
\noindent
where users write a \code{defTests} function:
...
a tester instance runs the Circuit Under Test (CUT) is as a process
with stdin and stdout connected so that inputs can be sent to the CUT and outputs can
...
% \begin{scala}
% class CrossbarIo(n: Int) extends Bundle {
%   val in  = Vec(n){ Bits(w, INPUT) }
%   val sel = Vec(n){ UFix(sizeof(n),  INPUT) }
%   val out = Vec(n){ Bits(w, OUTPUT) }
% }
% \end{scala}
