...
object tutorial {
  def main(args: Array[String]) = {
    chiselMainTest(args ++ Array("--gen-harness"), 
      ...
      c => Printer("%= %= %= %=", c.io.sel, c.io.in0, c.io.in1, c.io.out));
  }
}
\end{example}
...
%                 // Default: 0 (Combinational)
%           // A memory with no ports, with data width derived from
%           another signal.
%  // Data formatted for Verilog readmemh

% This example can be written more compactly as:
% 
% \begin{example}
% val regfile = Mem(64, io.wen, io.addr_in, io.data_in1, w_mask = io.bit_mask,
