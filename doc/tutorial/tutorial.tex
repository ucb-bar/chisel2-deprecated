...
\hline
\hline
\multicolumn{2}{|l|}{Arithmetic operations.  Valid on Nums: Fix and UFix. } \\
\hline
...
\hline
\hline
\multicolumn{2}{|l|}{Arithmetic comparisons.  Valid on Nums: Fix and
  UFix. Returns Bool.} \\
\hline
...
%%              f: UFix, g: UFix, h: UFix, i: UFix): Bool =
...
%circuits by turning them into object-oriented components.  Unlike
%functional generators, components also provide a coarse hierarchy on a
...
sequentially, we have provided a \verb+Wire+ constructor to provide
declaration of a wire node that can be used immediately, but whose
...
\noindent
The type given in braces following the \verb!Wire! constructor allows
the Scala compiler to know the types of \verb!pcPlus4! and
\verb!branchTarget! before they are defined.  The wiring operator
\verb!:=! is then used to wire up
...
Cascaded conditional updates are prioritized from bottom to top
following semantics of assignments to variables in Scala.  In particular, the last conditional
update whose condition evaluates to true is chosen to execute its
updates.  For example,
\begin{example}
when (c1)   { r := Bits(1) }
when (c2)   { r := Bits(2) }
...
viewed as adding a mux to the input of a register, where the mux
select is controlled by the \code{when} predicate such that the update
expression is muxed into the register when the predicate is true, else
the input comes from following code.  In addition, the predicate is
OR-ed into a firing signal that drives the load enable of the
register.  Subsequent \code{when} constructs chain on to the input of
the earlier muxes and on to the OR chain.  The compiler adds
termination values to the end of the chain so that if no conditional
updates fire in a clock cycle, the load enable of the register will be
deasserted and the register value will not change.
...
Simple memories allow users to define simple single write multiple
read memories.  The basic creation of the memory object looks as follows:
...
\noindent
where \verb+n+ is the number is the depth, \verb+wrEnable+ says
whether writes occur, \verb+wrAddr+ provide the address to write to,
and \verb+wrData+ give the data to write.  This memory object can then be
read from using the read method with a read address.  For
example, an audio recorder could be defined as follows:
...
The device records while the button is held down and plays back when it is released.
...
The following is an example of a memory with
one read and write port and the setting of additional parameters to
mimic common static memory (SRAM) behaviors:

%% TODO: CHANGE TO CURRIED TYPE 
\begin{example}
Mem.setDefaultReadLatency(1);                 // Default: 0
val regfile = Mem(64){ io.data_in1.clone }; // A memory with no ports, with data width derived from another signal.
regfile.setHexInitFile("hex_init_values.txt"); // Data formatted for Verilog readmemh
regfile.write(io.addr_in, io.data_in1, io.wen, wrMask = io.bit_mask);
...
\begin{example}
val regfile = Mem(64, io.wen, io.addr_in, io.data_in1, wrMask = io.bit_mask, readLatency = 1, hexInitFile = "hex_init_values.txt");
...
reference to a verilog instance of a memory module, add
\verb+regfile.setTarget('inst)+.  When Chisel compiles to Verilog, a
second file will be generated, eg: \verb+design.conf+, which can be
used by the synthesis design flow to construct the requested memory objects.

In addition to read and write memory ports, the \verb+Mem+ memory object supports
combined read/write ports.  The following example defines a memory with two read/write
ports:

\begin{example}
val rw_mem = Mem(64){ io.data_in1.clone };
...
io.data_out2 := rw_mem.rw(io.addr_in,
                          ...
                          wrMask = ~Bits(0,32), // Optional write mask
...
%   val sel = Vec(n){ UFix(width = (log(n)/log(2)).toInt, dir = INPUT) };
%   val out = Vec(n){ Bits(width = w, dir = OUTPUT) };
% }
% \end{example}
...
\begin{figure}
\centerline{\includegraphics[width=3.5in]{figs/cpu.png}}
...
% Sometimes hierarchical and delegational subinterfaces are awkward to use.
...
% A Bool can be automatically treated as a single bit UFix (with values
% 0 or 1), but an Int or UFix cannot be used as a Bool without an
% explicit cast.
%
...
%
% \begin{example}
% Lit(10, 4) // means a constant node of type Bits that is 4 bits
...
%
% Component correspond to Verilog modules
...
%   val out = Bits(1, OUTPUT);
% };
% def Mux2(sel: Bits, in0: Bits, in0: Bits): Bits = {
%   val m = new Mux2();
...
% 
% \noindent
% where the two thunk return types are parameterized to be a type \code{T} that is a subclass of \code{Bits}.
