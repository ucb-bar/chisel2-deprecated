...
emulator                \comment{\# chisel emulator source code}
...
tutorial/emulator       \comment{\# tutorial emulator files}
tutorial/runall.sh      \comment{\# runs tutorial tests}
\end{FramedSemiVerb}
\end{frame}
...
\begin{frame}[fragile]{Real Filter Assignment}
\begin{itemize}
...
class FanoutIO(n: Int) extends Bundle {
  ...
  override def clone() = (new FanoutIO(n)).asInstanceOf[this.type]
}
\end{scala}

\noindent
where \verb+Vec+ takes a size as the first argument and a block
returning a port as the second argument and \code{clone} binds the
construction argument \code{n} to cloning.
