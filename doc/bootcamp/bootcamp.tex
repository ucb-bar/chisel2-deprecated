...
\begin{center}
\includegraphics[height=0.2\textheight]{../tutorial/figs/DUT.pdf}
\end{center}

\column{0.45\textwidth}
{\lstset{basicstyle={\tiny\ttfamily}}
\begin{scala}
class Testy[+T <: Module] (val c: T) {
  // set by expect or user to determine test result
  var ok: Boolean
  // available for random testing
  val rnd: Random
  def reset(n: Int = 1)
  def step(n: Int): Int
  // users use flatten / litValue to convert to BigInts
  def poke(data: Mem[T], index: Int, x: BigInt): Unit
  def poke(data: Bits, x: BigInt): Unit
  def poke(data: CompositeData, x: Array[BigInt]): Unit
  def peek(data: Mem[T], index: Int): Unit
  def peek(data: Bits): BigInt
  def peek(data: CompositeData): Array[BigInt]
...
\begin{tiny}
\begin{itemize}
\item user subclasses \code{Testy} specifying DUT as param + tests in body
\item \code{poke} sets values,
\item \code{step} cycles the circuit one time unit,
\item \code{peek} reads values,
\item \code{expect} compares peeked circuit value to expected arg
\end{itemize}
\end{tiny}
...
\begin{frame}[fragile]{Testbench Ingredients}
...
\begin{itemize}
\item need to check outputs directly using \verb+peek+
\end{itemize}
\begin{scala}
