...
\verb$PriorityMux(in:Iterable[(Bits, Bool]): Bits$ \newline
\verb$PriorityMux(sel:Bits/Iterable[Bool],$ \newline
\verb$            in:Iterable[Bits]): Bits$ \newline
\begin{tabular}{l l l}
& \multicolumn{2}{l}{A mux with multiple select signals, prioritizing the first} \\
& \verb$in$ & \verb$Iterable[(Bits, Bool)]$ combined input and select \\
& \verb$in$ & \verb$Iterable[Bits]$ mux inputs \\
