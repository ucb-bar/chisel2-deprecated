...
\begin{center}
\includegraphics[scale=0.02]{bigduck} Version 0.4 (beta): \today
\end{center}
...
& \verb$gen$ & initial element data, called once per element \\
\end{tabular}
...
\section{Standard Library: Functions } \hrulefill
\subsection{Math and Hardware Generation Helpers}: \newline
\verb$log2Up(in:Int): Int$ \newline
\phantom{x} $log_2(\texttt{in})$ rounded up \newline
\verb$log2Down(in:Int): Int$ \newline
\phantom{x} $log_2(\texttt{in})$ rounded down \newline
\verb$isPow2(in:Int): Boolean$ \newline
\phantom{x} \verb$True$ if \verb$in$ is a power of 2

\subsection{Functional Blocks, Stateless}: \newline
\verb$PopCount(in:Bits/Seq[Bool]): UInt$ \newline
\begin{tabular}{l l l}
& \multicolumn{2}{l}{Returns number of bits set (= 1) in \texttt{in}} \\
& \verb$in$ & input to count \verb$1$s \\
\end{tabular}

\verb$Reverse(in:UInt): UInt$ \newline
\begin{tabular}{l l l}
& \multicolumn{2}{l}{Reverses the bit order of \texttt{in}} \\
& \verb$in$ & input to reverse \\
\end{tabular}

\verb$UIntToOH(in:UInt, width:Int): Bits$ \newline
\begin{tabular}{l l l}
& \multicolumn{2}{l}{Returns the one-hot encoding of \texttt{in}} \\
& \verb$in$ & numerical input \\
& \verb$width$ & {\em(optional)} output width (inferred if unspecified) \\
\end{tabular}

\verb$OHToUInt(in:Bits/Seq[Bool]): UInt$ \newline
\begin{tabular}{l l l}
& \multicolumn{2}{l}{Returns the numerical representation of one-hot \texttt{in}} \\
& \verb$in$ & one-hot input \\
\end{tabular}
...
\begin{tabular}{l l l}
& \multicolumn{2}{l}{Returns the position the least significant \texttt{1} in \texttt{in}} \\
& \verb$in$ & input bitvector \\
\end{tabular}

\verb$PriorityEncoderOH(in:Bits): UInt$ \newline
\begin{tabular}{l l l}
& \multicolumn{2}{l}{Returns the position of the hot bit in \texttt{in}} \\
& \verb$in$ & one-hot input \\
\end{tabular}

\subsection{Functional Blocks, Stateful}: \newline
...
\begin{tabular}{l l l}
& \multicolumn{2}{l}{Shift register, returns $n$-cycle delayed input} \\
& \verb$n$ & delay cycles \\
& \verb$in$ & input to delay \\
& \verb$en$ & {\em(optional, default True)} enable \\
\end{tabular}

\section{Standard Library: Interfaces } \hrulefill
\subsection{DecoupledIO } is a \verb$Bundle$ with a ready-valid interface,\newline
...
\subsubsection{Interface}: \newline
\begin{tabular}{l l l}
& \verb$.io.in$ & \verb$Vec$ of \verb$DecoupledIO$ inputs \\
...
\subsubsection{Interface}: \newline
\begin{tabular}{l l l}
& \verb$.io.enq$ & \verb$DecoupledIO$ source \\
...
\subsubsection{Interface}: \newline
\begin{tabular}{l l l}
& \verb$.io.enq$ & \verb$ValidIO$ source \\
& \verb$.io.deq$ & \verb$ValidIO$ sink \\
\end{tabular}

\section{Testing Library } \hrulefill
