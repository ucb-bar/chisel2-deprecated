...
\end{center}
\caption{DUT run using a Testy object in Scala with stdin and stdout connected}
\label{fig:dut}
\end{figure}
...
subclasses of the \code{Testy} class:
...
class Testy[T <: Module] (val c: T) {
  ...
  def reset(n: Int = 1)
  def step(n: Int): Int
  def poke(data: Mem[T], index: Int, x: BigInt)
  def poke(data: Bits, x: BigInt)
  def poke(data: Aggregate, x: Array[BigInt])
  def peek(data: Mem[T], index: Int)
