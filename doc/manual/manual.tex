...
\begin{scala}
val r1 = Reg(next = io.in)
val r2 = Reg(init = UInt(1, 8))
val r3 = Reg(next = io.in, init = UInt(1))
...
The clock for state elements and modules can be specified using an additional named parameter called clock:
...
There are two ways that circuits can be defined to move data between clock domains.
...
The second and more general technique to send data between domains (that can be constructed using the primitive one bit scheme) is an asynchronous queue:
...
\noindent
Using these queues, we can then move a signalA from clock domains clockA to signalB in clockB by specifying the standard queue parameters and the two clocks and then by using the standard decoupled ready/valid signals:
...
In the C++ backend, for every clock \verb+i+ Chisel produces
\begin{itemize}
\item a \verb+uint64_t clk_i+ field representing the clock \verb+i+'s period,
\item a \verb+uint63_t clk_i_cnt+ field representing the clock \verb+i+'s current count, and
\item \verb+clock_lo_i+ and \verb+clock_hi_i+ methods
\end{itemize}

\noindent
Next for each application, Chisel produces
\begin{itemize}
\item an \verb+int reset()+ function which ensures that all \verb+clock_lo+ and \verb+clock_hi+ functions are called at least once, and
\item an \verb+int clock(reset)+ function which computes min delta, invokes appropriate \verb+clock_lo+ and \verb+clock_hi+'s and returns min delta used.
\end{itemize}
...
\begin{itemize}
\item produces a new port for each clock / reset, and
\item wires all the clocks to the top module.
\end{itemize}

\noindent
A user must then create an \verb+always+ block clock driver for every clock \verb+i+.
...
def Enum[T <: UInt](type: T, n: Int): List[T]

// create enum values of given type and names
def Enum[T <: UInt]
  (type: T, l: Symbol *): Map[Symbol, T]

// create enum values of given type and names
def Enum[T <: UInt]
  (type: T, l: List[Symbol]): Map[Symbol, T]
\end{scala}
