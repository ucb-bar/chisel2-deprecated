...
\begin{scala}
object RAM {
  def apply[T <: Data](depth: Int)(type: => T): RAM
}

class RAM[T <: Data](n: Int, type: () => T) 
    extends Updateable {
  def apply(idx: UFix): T
}

class RAMRead[T <: Data]
      (val ram: RAM, val idx: UFix) extends Node {
  override def := (data: T): T
}

class RAMWrite[T <: Data]
      (val ram: RAM, val idx: UFix, val kind: T) 
    extends Node
\end{scala}
