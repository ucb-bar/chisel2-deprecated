...
\begin{scala}
class Tester
  (val vecs: Array[Array[BigInt]], 
   val bindings: Seq[Data])

object chiselMainTest {
  def apply[T <: Component]
    (args: Array[String], comp: () => T)(
     tester: T => Tester, 
     printer: T => TestIO = null)
}
\end{scala}

\noindent
where users specify a \code{Tester} object consisting of test vectors and
test vector bindings.
Test vectors are sequences of \code{BitInt}s, 
where  each test vector has input in the first part of the sequence
and non-input in the remaining part of the sequence.
The test vector bindings are nodes specified to
\code{Tester} that are flattened and split into input and non-input leaves.
