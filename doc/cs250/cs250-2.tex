...
\noindent
where \verb+Enum(UInt(), 2)+ creates a list of two \code{UInt()} literals.
...
{\lstset{basicstyle={\scriptsize\ttfamily}}
\begin{scala}
class Filter (isOk: (UInt) => Bool) 
    extends Module { 
  val io  = new ValidUInt()
  io.out.data  := io.in.data
  io.out.valid := 
...
val odds   = 
  Module(new Filter((x) => x & UInt(1)))
val smalls = 
  Module(new Filter((x) => x < UInt(10)))
\end{scala}
}
...
\begin{scala}
class DecoupledUInt extends ValidUInt { 
...
From there we can define a filter interface by nesting two
\verb+ValidUInt+s into a new \verb+FilterIO+ bundle:
...
Unfortunately, as defined, decoupled interfaces are defined only for 16 bit \verb+UInt+s.  
...
\begin{scala}
val ufix32s = new Valid( UInt(width = 32) )
...
\noindent
We can now define \code{DecoupledIO} on arbitrary data types:

\begin{scala}
val ufix32s = new DecoupledIO( UInt(width = 32) )
val pkts    = new DecoupledIO( new Packet() )
\end{scala}
}
\end{frame}
...
class SmallOdds[T <: Data](type: T) extends Module { 
  val io     = new FilterIO()
  val smalls = Module(new Filter(_ < UInt(10), type ))
  val q      = Module(new Queue(type))
  val odds   = Module(new Filter(_ & UInt(1), type ))
...
val block = new SmallOdds( UInt(width = 32) )
\end{scala}
}
\end{frame}
