...
Ths document is intended to familiarize you with component instantiation and using primitives in Chisel.  The tutorial is broken into small modules each of which will present a new feature of Chisel. This tutorial assumes that you are familiar with what was presented in the previous tutorial.
...
Like other hardware description languages, Chisel allows fairly straightforward module instantiation to enable modularity and hierarchy. In Chisel, instantiating a Module class is the equivalent to instantiating a module in Verilog. To do this, we simply use the keyword \verb+new+ in order to indicate that we are instnatiation a new module. We want to make sure we assign this to a value so that we can reference its input and outputs which we also need to connect.
...
class Adder4 extends Module {
  ...
  //Adder for bit 0
  val Adder0 = new FullAdder()
  Adder0.io.a   := io.A(0)
  Adder0.io.b   := io.B(0)
  Adder0.io.cin := io.Cin
  val s0 = Adder0.io.sum
  //Adder for bit 1
  val Adder1 = new FullAdder()
  Adder1.io.a   := io.A(1)
  Adder1.io.b   := io.B(1)
  Adder1.io.cin := Adder0.io.cout
  val s1 = Cat(Adder1.io.sum, s0)
  //Adder for bit 2
  val Adder2 = new FullAdder()
  Adder2.io.a   := io.A(2)
  Adder2.io.b   := io.B(2)
  Adder2.io.cin := Adder1.io.cout
  val s2 = Cat(Adder2.io.sum, s1)
  //Adder for bit 3
  val Adder3 = new FullAdder()
  Adder3.io.a   := io.A(3)
  Adder3.io.b   := io.B(3)
...
The \verb+Vec+ class allows you to create an indexable vector in Chisel which can be filled with any primitive or module. The general syntax for a \verb+Vec+ declaration is given by:
\begin{scala}
val myVec = Vec.fill( <number of elements> ) { <data type> }
\end{scala}
...
The syntax for the \verb+Vec+ class is slightly different when instantiating a vector of components. When instantiating a vector of components the data type that is specified in the {} braces is slightly different than the usualy primitive types. To specify a vector of components, we use the \verb+io+ bundle when specifying the type of the vector. For example, in order to specify a \verb+Vec+ with 16 components , say \verb+FullAdder+s in this case, we would use the following declaration:

\begin{scala}
val FullAdders = Vec.fill(16){ new FullAdder().io }
\end{scala}

Notice we use the keyword \verb+new+ in the vector definition before the component name \verb+FullAdder+. For how to actually access the \verb+io+ on the vector components, refer to the next section.

\section{Parametrization}

In the previous Adder example, we explicitly instantiated four different copies of a \verb+FullAdder+ and wired up the ports. But suppose we want to generalize this structure to an n-bit adder. Like Verilog, Chisel allows a you to pass parameters to specify certain aspects of your design. In order to do this, we add a field in the Module declaration to our Chisel definition.
...
class Adder(n:Int) extends Module {
  ...
  val FAs = Vec(n) {new FullAdder().io}

  //define carry and sum wires
  val carry = Vec(n+1) {UInt(width = 1)}
  val sum   = Vec(n) {Bool()}  
...
Instantiating a parametrized module is very similar to instantiating an unparametrized module except that we must provide arguments for the parameter values. For instance, if we wanted to instantiate a 4-bit version of the \verb+Adder+ component we defined above, it would look like:

\begin{scala}
val adder4 = new Adder(4)
\end{scala}

We can also instantiate the \verb+Adder+ by explicitly specifying the value of it parameter \verb+n+ like the this:

\begin{scala}
val adder4 = new Adder(n = 4)
\end{scala}

Explicitly specifying the parameter is useful when you have a module with multiple parameters. Suppose you have a parametrized FIFO module with the following component definition:
...
\begin{scala}
val FIFO1 = new FIFO(16, 32)
val FIFO2 = new FIFO(width = 16, depth = 32)
val FIFO3 = new FIFO(depth = 32, width = 16)
\end{scala}

All of the above definitions pass the same parameters to the FIFO component. Notice that when you explicitly assign the parameter values, they can occur in any order you want such as the definition for FIFO3.
...
We see that clearly it is much cleaner to use the primitive \verb+Mux+ type instead of trying to write and implement our own general multiplexor since the \verb+Mux+ type does all the bookkeeping for you.
